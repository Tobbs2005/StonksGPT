...
Be specific and accurate. If you find ticker symbols, format them clearly (e.g., "Ticker: AAPL" or "NYSE: AAPL").`;
    ...
    if (error.message?.includes('GEMINI_API_KEY')) {
      return `Search error: ${error.message}. Please set GEMINI_API_KEY in your .env file.`;
    }
    
    // If model not found, provide helpful error
    if (error.message?.includes('404') || error.message?.includes('not found')) {
      return `Search error: Gemini model not found. Please check your API key has access to Gemini models. Error: ${error.message}`;
    }
    
    return `Search error: ${error.message}. Please try using the get_asset tool with a specific symbol instead.`;
  }
}
...
Be specific and accurate. Only include valid ticker symbols.`;
    ...
    return `Error searching for symbols: ${error.message}`;
  }
}
