...
export function checkEnvironment(): { valid: boolean; errors: string[] } {
  ...
  const envPaths = possiblePaths.filter((possiblePath) => fs.existsSync(possiblePath));

  if (envPaths.length > 0) {
    console.log(`Found .env file(s) at: ${envPaths.join(', ')}`);
    for (const envPath of envPaths) {
      // Try to load each .env file and fill missing keys
      try {
        ...
        if (loadedKeys > 0) {
          console.log(`Loaded ${loadedKeys} API key(s) from ${envPath}`);
        }
      } catch (err) {
